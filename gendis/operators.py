...
def position_based_crossover(ind1, ind2, p=0.25):
    """Apply position-based crossover on shapelet sets"""
    new_ind1, new_ind2 = [], []
    ind1 = np.array(ind1).reshape(-1, 1)
    ind2 = np.array(ind2).reshape(-1, 1)

    max_length = max(len(ind1), len(ind2))
    min_length = min(len(ind1), len(ind2))

    for i in range(max_length):
        if i < min_length and len(ind1[i]) > 1 and len(ind2[i]) > 1 and np.random.random() < p:
            # Randomly choose a crossover point within each shapelet
            crossover_point1 = np.random.randint(1, len(ind1[i]))
            crossover_point2 = np.random.randint(1, len(ind2[i]))

            # Perform crossover based on the chosen points
            new_shap1 = np.hstack((ind1[i][:crossover_point1], ind2[i][crossover_point2:]))
            new_shap2 = np.hstack((ind2[i][:crossover_point2], ind1[i][crossover_point1:]))

            new_ind1.append(new_shap1)
            new_ind2.append(new_shap2)
        else:
            if i < len(ind1):
...
def crossover_operator(ind1, ind2, crossover_prob=1, decay_factor=0.9):
    """Crossover operator with probability decay for individuals that have already been crossed over"""
    new_ind1, new_ind2 = [], []

    # Check if individuals have different lengths
    max_length = max(len(ind1), len(ind2))
    min_length = min(len(ind1), len(ind2))

    # Initialize crossover probabilities for each individual
    ind1_crossover_prob = crossover_prob
    ind2_crossover_prob = crossover_prob

    # Adjust crossover probability based on whether individuals have been crossed over
    if np.random.random() < decay_factor:
        # Reduce crossover probability if individual has been crossed over
        ind1_crossover_prob *= decay_factor
    if np.random.random() < decay_factor:
        # Reduce crossover probability if individual has been crossed over
        ind2_crossover_prob *= decay_factor

    # Perform crossover
    if np.random.random() < ind1_crossover_prob:
        # Perform crossover for individual 1
        crossover_point = np.random.randint(1, min_length)
        new_ind1 = ind1[:crossover_point] + ind2[crossover_point:]
    else:
        new_ind1 = ind1

    if np.random.random() < ind2_crossover_prob:
        # Perform crossover for individual 2
        crossover_point = np.random.randint(1, min_length)
